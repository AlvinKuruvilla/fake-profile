...
def generate_couple(datasets):
    for i in range(len(datasets)):
        ...
        while (index < length):
            if (index == length - 1):
                datasets[i].loc[index + 0.5] = [datasets[i].at[index, "event_time"],
                                                1 - int(datasets[i].at[index, "press_type"]),
                                                datasets[i].at[index, "key_code"], datasets[i].at[index, "user_id"]]
                datasets[i] = datasets[i].sort_index().reset_index(drop=True)
                check_couple_order(datasets[i], index)
                break
            else:
                if ((datasets[i].at[index, "key_code"] != datasets[i].at[index + 1, "key_code"]) or (
                        datasets[i].at[index, "press_type"] == datasets[i].at[index + 1, "press_type"])):
                    datasets[i].loc[index + 0.5] = [datasets[i].at[index, "event_time"],
                                                    1 - int(datasets[i].at[index, "press_type"]),
                                                    datasets[i].at[index, "key_code"], datasets[i].at[index, "user_id"]]
                    datasets[i] = datasets[i].sort_index().reset_index(drop=True)
...
def keystroke_feature_extract(keystroke):
    ...
    for i in range(keystroke.shape[0]):
        ...
        keystroke.loc[i, ["hl", "di_ud", "di_dd", "di_uu", "di_du", "tri_ud", "tri_dd", "tri_uu", "tri_du", "key"]] = [
...
def embed_zero_padding(sequence, sequence_length):
    ...
    new_items_df = pd.DataFrame([[0] * sequence.shape[1] for i in range(missing_sample_count)],
        columns=list(sequence.columns))
...
def sync_imu_data(accelerometer_data, gyroscope_data, magnetometer_data, sync_period, imu_sequence_length):
    ...
    while start_time < highest_time:
        relevant_accelerometer_data = accelerometer_data.loc[
            (accelerometer_data['event_time'] >= start_time) & (accelerometer_data['event_time'] <= end_time)]
        relevant_gyroscope_data = gyroscope_data.loc[
            (gyroscope_data['event_time'] >= start_time) & (gyroscope_data['event_time'] <= end_time)]
        relevant_magnetometer_data = magnetometer_data.loc[
            (magnetometer_data['event_time'] >= start_time) & (magnetometer_data['event_time'] <= end_time)]

        if (relevant_accelerometer_data.shape[0] == 0 or relevant_gyroscope_data.shape[0] == 0 or
                relevant_magnetometer_data.shape[0] == 0):
...
def pre_process(event_data, event_sequence_length, imu_sequence_length, offset, accelerometer_data, gyroscope_data,
    ...
    for sequence in event_sequences:
        ...
        imu_sequence = sync_imu_data(relevant_accelerometer_data, relevant_gyroscope_data, relevant_magnetometer_data,
            sync_period, imu_sequence_length)

        imu_sequences.append(imu_sequence)

    event_sequences[len(event_sequences) - 1] = embed_zero_padding(event_sequences[len(event_sequences) - 1],
...
def read_keystroke(absolute_path, users_list):
    ...
    for userid in users_list:
        ...
        for session in os.listdir(dataset_complete_path / f"{userid}"):
            if ("." not in session):
                keystroke_csv_data = pd.read_csv(dataset_complete_path / f"{userid}/{session}/KeyPressEvent.csv",
                    header=None, usecols=[0, 3, 4], names=["event_time", "press_type", "key_code"])
                if (keystroke_csv_data.shape[0] != 0):
                    ...
                    accelerometer_csv_data = pd.read_csv(
                        dataset_complete_path / f"{userid}/{session}/Accelerometer.csv", header=None,
                        usecols=[0, 3, 4, 5], names=["event_time", "x", "y", "z"])
                    gyroscope_csv_data = pd.read_csv(dataset_complete_path / f"{userid}/{session}/Gyroscope.csv",
                        header=None, usecols=[0, 3, 4, 5], names=["event_time", "x", "y", "z"])
                    magnetometer_csv_data = pd.read_csv(dataset_complete_path / f"{userid}/{session}/Magnetometer.csv",
                        header=None, usecols=[0, 3, 4, 5], names=["event_time", "x", "y", "z"])
                    ...
                    keystroke_sequences, imu_sequences = pre_process(keystroke_csv_data, keystroke_sequence_len,
                        imu_sequence_len, windowing_offset, accelerometer_csv_data, gyroscope_csv_data,
                        magnetometer_csv_data)
...
if __name__ == "__main__":
    ...
    keystroke_sequence_len = 50 if config_data["keystroke_sequence_len"] is None else config_data[
        "keystroke_sequence_len"]
    ...
    validation_user_list, testing_user_list = train_test_split(val_test_user_list, test_size=15, train_size=15,
        shuffle=True)
